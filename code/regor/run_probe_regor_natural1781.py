...
from einfo_online_artifact import artifact_filename, load_online_artifact, reconstruct_sampled_arrays, restore_numpy_rng_state, validate_manifest
...
def main() -> None:
    ...
    parser.add_argument(
        "--head-root",
        type=Path,
        default=REPO_ROOT / "output" / "regor" / "2026-08-09_EInfo_Online_Artifacts",
    )
    parser.add_argument("--tail-root", type=Path, required=True)
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=REPO_ROOT / "output" / "regor" / "2026-08-23_probe_regor" / "online_predictions",
    )
